...
public class AutoGeneratedLevels : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        tempPosition.z += StepBetweenLevels;
        for (int i = 1; i < LevelAmount - 1; i++)
        {
            var tempGameObject = Levels[Random.Range(0, Levels.Length)];

            tempGameObject = Instantiate(tempGameObject);
            tempGameObject.SetActive(true);

            tempGameObject.transform.position = tempPosition;
            tempPosition.z += StepBetweenLevels;
        }
        ...
        LastLevel.transform.position = tempPosition;
    }
}
